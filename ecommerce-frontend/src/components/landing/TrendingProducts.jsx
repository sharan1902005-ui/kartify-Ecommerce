...
import { ShoppingCart, Star, ArrowRight, TrendingUp, Heart } from "lucide-react";
...
function ProductCard({ product, index }) {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -8, boxShadow: "0 24px 60px rgba(15,118,110,0.12)" }}
    ...
    >
      {/* Image */}
      <div className="relative bg-gradient-to-br from-slate-50 to-teal-50/30 h-52 flex items-center justify-center p-6 overflow-hidden">
        {discount > 0 && (
          <div className="absolute top-3 left-3 bg-red-500 text-white text-[11px] font-bold px-2.5 py-1 rounded-lg z-10">
            {discount}% OFF
          </div>
        )}
        <motion.button
          ...
          className="absolute top-3 right-3 w-8 h-8 rounded-xl bg-white/90 backdrop-blur-sm flex items-center justify-center z-10 shadow-sm"
        >
...
function SkeletonCard() {
  return (
    <div className="bg-white rounded-3xl overflow-hidden animate-pulse" style={{ border: "1px solid rgba(0,0,0,0.06)" }}>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            onClick={() => navigate("/products")}
            className="flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-sm border-2 border-teal-600 text-teal-600 hover:bg-teal-600 hover:text-white transition-colors shrink-0"
          >
          ...
          >
            {loading
              ...
              : filtered.length > 0
                ...
                : <div className="col-span-4 text-center py-16 text-slate-400 font-medium">No products found in this category.</div>
            }
          </motion.div>
        </AnimatePresence>
      </div>
