...
function ReviewCard({ review }) {
  return (
    <div className="relative p-7 rounded-3xl flex flex-col gap-4 shrink-0 w-80 h-full"
      style={{ background: "white", border: "1px solid rgba(0,0,0,0.06)", boxShadow: "0 4px 24px rgba(0,0,0,0.05)" }}>
      <Quote size={22} color={review.color} style={{ opacity: 0.15 }} className="absolute top-5 right-5" />
...
export default function Testimonials() {
  ...
  const prev = () => setFeatured((c) => (c - 1 + reviews.length) % reviews.length);
  const next = () => setFeatured((c) => (c + 1) % reviews.length);
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8">
        ...
        >
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-slate-600 font-bold ml-2 text-sm">4.9 · 10,000+ reviews</span>
          </div>
        </motion.div>
          ...
          <div className="relative">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                style={{
                  background: "linear-gradient(135deg, rgba(15,118,110,0.04), rgba(16,185,129,0.03))",
                  border: "1px solid rgba(15,118,110,0.1)",
                  boxShadow: "0 8px 48px rgba(15,118,110,0.07)",
                }}
              >
                <Quote size={36} color="#0F766E" style={{ opacity: 0.12 }} className="mx-auto mb-5" />
            ...
            {/* Nav buttons */}
            <button onClick={prev}
              className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-5 w-10 h-10 rounded-full bg-white shadow-lg border border-slate-100 flex items-center justify-center hover:bg-teal-50 hover:border-teal-200 transition-all">
              <ChevronLeft size={16} className="text-slate-600" />
            </button>
            <button onClick={next}
              className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-5 w-10 h-10 rounded-full bg-white shadow-lg border border-slate-100 flex items-center justify-center hover:bg-teal-50 hover:border-teal-200 transition-all">
